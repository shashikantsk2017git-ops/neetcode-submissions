class Solution {
    ...
    public List<String> findAll3(String[] words, List<String> out) {
        ...
        for(String str: words) {
            ...
            if(canFormLoop(str, set)) out.add(str);
        }
    ...
    public boolean canFormLoop(String str, Set<String> set) {
        boolean[] dp = new boolean[str.length() + 1];
        dp[str.length()] = true;  
        for (int ind = str.length() - 1; ind >= 0; ind--) {
            for (int i = ind; i < str.length(); i++) {  
                String prefix = str.substring(ind, i + 1);
                if (set.contains(prefix) && !prefix.equals(str) && dp[i + 1]) {
                    dp[ind] = true;
                    break; 
                }
            }
