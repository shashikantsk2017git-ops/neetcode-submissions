class Solution {
    ...
    //We are checking row i value or row i+1 value, consider curr = ith row and prev = i+1th row
    // 2D version	1D version
    // dp[i+1][j+1]	prev[j+1] (Next row i+1 means the row previouly computed so prev)
    // dp[i+1][j]	prev[j]
    // dp[i][j+1]	curr[j+1] (Current row ith means row currently computing so curr)
    // dp[i][j]	    curr[j]
        ...
        for (int ind1 = str1.length() - 1; ind1 >= 0; ind1--) {
            ...
            for (int ind2 = str2.length() - 1; ind2 >= 0; ind2--) {
                ...
                if (str1.charAt(ind1) == str2.charAt(ind2)) {
                    ...
                    dp[ind2] = Math.max(dp[ind2 + 1], dp[ind2]);
                prev = temp;
            }
        }
        return dp[0];
