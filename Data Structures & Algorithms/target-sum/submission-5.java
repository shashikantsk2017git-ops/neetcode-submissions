class Solution {
    ...
    private int countWays(int[] nums, int target) {
        ...
        // total possible sum range is [-totalSum, totalSum]
        int totalSum = 0;
        for (int num : nums) totalSum += num;

        if (Math.abs(target) > totalSum) return 0; // target unreachable
        ...
        dp[n][target + offset] = 1;          // base case: real sum == target at ind == n
        ...
        return dp[0][offset]; // real sum == 0 at ind == 0
    }
}
