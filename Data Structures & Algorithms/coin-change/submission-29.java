class Solution {
    public int coinChange(int[] coins, int amount) {
        ...
        return coinCountLoop0(coins, amount);
    ...
    public int coinCountLoop(int[] coins, int target) {
        ...
        //check for using each coins and see if able to get 1 to target amount
        //and how many coins required to get that
        for(int ind = coins.length-1; ind >=0; ind--) {
            for(int amt = 1; amt <= target; amt++) {
                //take
                int take = -1;
                //current coin <= amt and prev dp value with prev amt and ind
                if(coins[ind] - amt <= 0 && dp[amt-coins[ind]][ind] != -1)
    ...
    public int coinCountLoop0(int[] coins, int target) {
        int n = coins.length;
        ...
        for (int amt = 1; amt <= target; amt++) {
            for (int ind = 0; ind < n; ind++) {
                int take = -1;
                if (coins[ind] <= amt && dp[ind][amt - coins[ind]] != -1) {
                    take = 1 + dp[ind][amt - coins[ind]];
                }

                int nottake = (ind > 0) ? dp[ind - 1][amt] : -1;

                if (take == -1) dp[ind][amt] = nottake;
                else if (nottake == -1) dp[ind][amt] = take;
                else dp[ind][amt] = Math.min(take, nottake);
            }
        }
    ...
    public int coinCountLoopOpt1(int[] coins, int target) {
