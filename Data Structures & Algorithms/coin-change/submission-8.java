class Solution {
        ...
        for (int ind = coins.length - 1; ind >= 0; ind--) {
            for (int amt = 1; amt <= target; amt++) {
                ...
                if (coins[ind] <= amt && dp[amt - coins[ind]][ind] != -1)
                    take = dp[amt - coins[ind]][ind] + 1;

                // not take
                int nottake = dp[amt][ind + 1];

                // assign to current cell, not dp[target][ind]
    ...
    for (int ind = coins.length - 1; ind >= 0; ind--) {
        for (int amt = 1; amt <= target; amt++) {
            ...
            if (take == -1) dp[amt] = nottake;
            else if (nottake == -1) dp[amt] = take;
            else dp[amt] = Math.min(take, nottake);
        }
    }
