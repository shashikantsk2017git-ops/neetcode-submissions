class Solution {
    ...
    private int maxRobOpt(int[] nums) {
        ...
        for(int ind = nums.length-1; ind >=0; ind--) {
            //choose first house to rob
            int first =  nums[ind] + prev2; //ind + 2 house

            //Not choosing first house to rob
            int notFirst = prev1; // ind + 1 house
            
            int temp = Math.max(first, notFirst); // ind house
            prev2 = prev1; 
            prev1 = temp; //temp will be ind+1 house value so replace with prev1
        }
        return prev1;
    }
}
